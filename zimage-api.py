...
MODEL_PATH = os.environ.get("MODEL_PATH", "/home/user/modelscope/Z-Image")
...
class ImageGenerationRequest(BaseModel):
    ...
    negative_prompt: Optional[str] = Field(default="", description="负面提示词，用于排除不想要的内容")
    ...
    num_inference_steps: Optional[int] = Field(
        default=50, ge=1, le=100, description="推理步数(1-100)，推荐28-50"
    )
    guidance_scale: Optional[float] = Field(
        default=4.0, ge=0.0, le=20.0, description="CFG引导强度(0.0-20.0)，推荐3.0-5.0"
    )
    cfg_normalization: Optional[bool] = Field(
        default=False, description="是否启用CFG归一化"
    )
...
for d in devices:
    print(f"Loading Z-Image model on {d} ...")
...
print("All Z-Image models loaded")
...
def worker(device: str):
    ...
    while True:
        ...
        cfg_normalization = task["cfg_normalization"]

        print(f"[{device}] processing {task_id} | {width}x{height} | seed={seed} | steps={num_inference_steps} | cfg={guidance_scale}")
