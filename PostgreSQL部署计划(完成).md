...
```env
POSTGRES_SUPERUSER_PASSWORD=替换为强超级用户密码
APP_DB_PASSWORD=替换为强应用账号密码
```
...
postgres-service/
  ...
  init/
    ...
    001_schema.sql            # 画布项目主数据表、索引
    002_triggers.sql          # updated_at 自动更新时间
    003_rls.sql               # 项目成员维度的 RLS 只读策略，默认预置但不启用
    004_grants.sh             # 授权应用账号访问业务表
  scripts/
    start.sh                  # 首次创建 .env 或启动服务并检查
    check.sh                  # 检查容器、连接和表结构
    backup.sh                 # 生成 pg_dump custom 格式备份
    restore-test.sh           # 恢复到 anime_canvas_restore 做演练
  data/                       # PostgreSQL 数据目录，不提交 Git
  backups/                    # 备份目录，不放到 Web 静态目录
```
...
初始化表结构已覆盖当前画布项目后端需要接入的主数据：

- `users` / `user_devices`：用户、设备指纹哈希。
- `projects` / `project_members`：项目归属、成员角色和权限隔离基础。
- `canvases`：画布快照 JSONB。
- `assets`：图片、音频、视频素材库。
- `tasks`：AI 生成任务、进度、输入输出和错误。
- `providers` / `models`：模型供应商和模型配置 JSONB。
- `system_events`：系统审计、运行日志和诊断事件。
- `yjs_updates` / `yjs_snapshots`：Yjs 增量日志和协作快照。
...
```bash
./scripts/restore-test.sh ./backups/备份文件.dump
```

建议服务器 crontab 每日执行 `backup.sh`，并按“最近 7 天每日、最近 4 周每周”的策略清理旧备份。
...
- 每个请求解析出当前用户上下文。
- 所有项目、画布、素材、任务查询都带用户或项目成员校验。
- 写入 `projects` 时同步写入 owner 的 `project_members`。
- 访问画布、素材、Yjs 历史时通过 `project_id` 或 `canvas_id` 反查项目成员。
- 使用 RLS 时，在事务内执行：
...
当前 RLS 脚本先提供 select 隔离策略；insert/update/delete 建议由后端应用层明确校验后执行，后续再按接口边界补充更细的 RLS 写策略。
...
- 新增数据库连接模块。（进行中：后端已新增 `DATA_BACKEND=postgres` 开关，默认 `json`，切换后通过 `DATABASE_URL` 连接服务器 PostgreSQL）
- 新增迁移脚本目录。（进行中：已新增 `npm run postgres:dry-run`，用于读取当前 `data/db.json` 并输出 PostgreSQL 映射后的表级数量和风险提示，不写数据库）
- 实现 users/projects/canvases/assets/tasks 的 PostgreSQL CRUD。（进行中：7C-1U 首版采用兼容当前业务的整库快照事务适配层，已覆盖 users/user_devices/project_members/projects/canvases/assets/tasks/providers/models/system_events/yjs_updates/yjs_snapshots；细粒度 SQL CRUD 后续逐步拆分）
- 保留 JSON 存储作为 fallback。（已完成：不设置 `DATA_BACKEND=postgres` 时继续使用 `data/db.json`）

本地客户端后端连接服务器 PostgreSQL 示例：
...
正式迁移前建议启用严格模式，遇到缺失项目、缺失画布、重复 ID 等风险时直接失败：
...
- 实现 dry-run 对账。（已完成：`npm run postgres:dry-run` 与 `postgres:migrate` 共用 `postgres-migration-utils` 风险分析，输出表级数量、素材/任务/模型分布、错误/警告/信息）
- 实现正式导入。（已完成脚本：`npm run postgres:migrate -- --apply --confirm replace-postgres`；默认只演练不写库，正式写入前会备份当前 PostgreSQL 快照到 `data/postgres-migration-backups`，写入后做表级数量对账）
- 实现导入后接口 smoke test。（已完成脚本：`npm run postgres:api-smoke`，在 API 切到 `DATA_BACKEND=postgres` 后验证 health、公开模型、项目/画布/素材读取和 Admin overview）
- 实现失败回滚说明。（进行中：当前脚本已生成 PostgreSQL 写入前快照备份；后续可补 `postgres:restore-snapshot` 一键恢复）
...
# 3. 正式迁移前严格模式阻断 warning
$env:POSTGRES_DRY_RUN_STRICT="true"; npm run postgres:dry-run

# 4. 连接 PostgreSQL 但只演练，不写库
npm run postgres:migrate

# 5. 正式写入 PostgreSQL
npm run postgres:migrate -- --apply --confirm replace-postgres

# 6. API 切换 DATA_BACKEND=postgres 后做接口 smoke
$env:API_BASE_URL="http://127.0.0.1:8787"; npm run postgres:api-smoke
```

如果 PostgreSQL 已有数据，正式导入默认拒绝覆盖；确认已经接受脚本生成的备份后再追加：

```powershell
npm run postgres:migrate -- --apply --confirm replace-postgres --allow-non-empty
```
